...
class Config(object):
    ...
    TIME_RELATED_LIST: List[int] = field(
        default_factory=lambda: [15 * ONE_MINUTE, ONE_HOUR, 3 * ONE_HOUR, 6 * ONE_HOUR, 12 * ONE_HOUR],
        init=False,
    )
...
class FeatureFactory(object):
    ...
    def process_all_sn(self) -> NoReturn:
        ...
        sn_files = [
            x for x in sn_files if
            x.replace("csv", "feather") not in exist_sn_file_list and x.endswith(self.config.DATA_SUFFIX)
        ]
        sn_files.sort()
...
if __name__ == "__main__":
    ...
    # 处理 A 和 B 两种类型
    for sn_type in ["A", "B"]:
        print(f"\nProcessing type {sn_type}...")

        # 为每种类型创建必要的目录
        type_specific_dirs = {
            'feature': os.path.join("./WWW2025/combined_sn_feature", f"type_{sn_type}"),
            'train': os.path.join("./WWW2025/train_data", f"type_{sn_type}"),
            'test': os.path.join("./WWW2025/test_data", f"type_{sn_type}_{test_stage}")
        }

        for dir_path in type_specific_dirs.values():
            os.makedirs(dir_path, exist_ok=True)

        # 检查数据源路径是否存在
        data_path = os.path.join("./stage2", f"type_{sn_type}")
        if not os.path.exists(data_path):
            print(f"Warning: Source data path {data_path} does not exist!")
            continue

        # 检查ticket文件是否存在
        if not os.path.exists("./stage2/failure_ticket.csv"):
            print("Error: failure_ticket.csv not found!")
            exit(1)

        # 初始化配置类 Config
        config = Config(
            data_path=data_path,
            feature_path=type_specific_dirs['feature'],
            train_data_path=type_specific_dirs['train'],
            test_data_path=type_specific_dirs['test'],
            test_data_range=test_data_range,
            ticket_path="failure_ticket.csv",
        )

        # 检查特征文件是否已存在
        feature_files = os.listdir(type_specific_dirs['feature'])
        if not feature_files:
            print("Generating features...")
            feature_factory = FeatureFactory(config)
            feature_factory.process_all_sn()
        else:
            print("Features already exist, skipping feature generation...")

        # 检查训练数据文件是否已存在
        train_files = ["positive_train.feather", "negative_train.feather"]
        if not all(os.path.exists(os.path.join(type_specific_dirs['train'], f)) for f in train_files):
        ...
        # 检查测试数据文件是否已存在
        test_files = os.listdir(type_specific_dirs['test'])
        if not any(f.startswith("res_") and f.endswith(".feather") for f in test_files):
        ...
        for sn in result:
            for timestamp in result[sn]:
                all_submissions.append([sn, timestamp, sn_type])

    # 创建submission_test目录
    os.makedirs("submission_test", exist_ok=True)
    ...
    # submission.to_csv(f"submission_test/submission{sn_type}.csv", index=False, encoding="utf-8")
    submission.to_csv(f"submission_test/submission.csv", index=False, encoding="utf-8")
